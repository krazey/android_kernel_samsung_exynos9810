...
{
	/*
	 * We just charge FS_MAX_KEY_SIZE bytes to the user's key quota for
	 * each key, regardless of the exact key size.  The amount of memory
	 * actually used is greater than the size of the raw key anyway.
	 */
	return key_payload_reserve(key, FS_MAX_KEY_SIZE);
}
...
#define FSCRYPT_MK_DESCRIPTION_SIZE	(2 * FS_KEY_IDENTIFIER_SIZE + 1)

#define FSCRYPT_MK_USERS_DESCRIPTION_SIZE	\
	(CONST_STRLEN("fscrypt-") + 2 * FS_KEY_IDENTIFIER_SIZE + \
	 CONST_STRLEN("-users") + 1)

#define FSCRYPT_MK_USER_DESCRIPTION_SIZE	\
	(2 * FS_KEY_IDENTIFIER_SIZE + CONST_STRLEN(".uid.") + 10 + 1)
...
static void format_mk_users_keyring_description(
			...
			const u8 mk_identifier[FS_KEY_IDENTIFIER_SIZE])
{
	sprintf(description, "fscrypt-%*phN-users",
		FS_KEY_IDENTIFIER_SIZE, mk_identifier);
}
			...
			const u8 mk_identifier[FS_KEY_IDENTIFIER_SIZE])
{
	...
	sprintf(description, "%*phN.uid.%u", FS_KEY_IDENTIFIER_SIZE,
			      ...
			      struct key *keyring)
{
	...
	if (mk_spec->type == FS_KEY_SPEC_TYPE_IDENTIFIER) {
...
int fscrypt_ioctl_add_key(struct file *filp, void __user *_uarg)
{
	...
	case FS_KEY_SPEC_TYPE_DESCRIPTOR:
		...
		if (!capable(CAP_SYS_ADMIN))
			goto out_wipe_secret;
		break;
	case FS_KEY_SPEC_TYPE_IDENTIFIER:
...
int fscrypt_verify_key_added(struct super_block *sb,
			     const u8 identifier[FS_KEY_IDENTIFIER_SIZE])
{
	...
	memcpy(mk_spec.u.identifier, identifier, FS_KEY_IDENTIFIER_SIZE);
...
static int do_remove_key(struct file *filp, void __user *_uarg, bool all_users)
{
	 ...
	 */
	if (arg.key_spec.type == FS_KEY_SPEC_TYPE_DESCRIPTOR &&
