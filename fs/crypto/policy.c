					...
					const struct inode *inode)
{
	...
	if (policy->flags & FS_POLICY_FLAG_DIRECT_KEY) {
		fscrypt_warn(inode,
			      ...
			      const struct inode *inode)
{
	...
	case FSCRYPT_POLICY_V1: {
		...
		if (policy->flags & ~(FS_POLICY_FLAGS_PAD_MASK |
	...
	case FSCRYPT_POLICY_V2: {
		...
		if ((policy->flags & FS_POLICY_FLAG_IV_INO_LBLK_64) &&
						...
						void *fs_data, bool preload)
{
	...
	BUILD_BUG_ON(sizeof(ctx) != FS_SET_CONTEXT_MAX_SIZE);
