...
void exynos_pcie_send_pme_turn_off(struct exynos_pcie *ep);
...
static int exynos_pcie_set_l1ss(int enable, struct pcie_port *pp, int id)
{
	...
	dev_info(pp->dev, "%s: START (state = 0x%x, id = 0x%x, enable = %d)\n",
			__func__, ep->l1ss_ctrl_id_state, id, enable);
	spin_lock_irqsave(&ep->conf_lock, flags);
	if (ep->state != STATE_LINK_UP || ep->atu_ok == 0) {
		if (enable)
			ep->l1ss_ctrl_id_state &= ~(id);
		else
			ep->l1ss_ctrl_id_state |= id;
		spin_unlock_irqrestore(&ep->conf_lock, flags);
		dev_info(pp->dev, "%s: It's not needed. This will be set later."
				"(state = 0x%x, id = 0x%x)\n",
				__func__, ep->l1ss_ctrl_id_state, id);
		return -1;
	}
...
{
	struct exynos_pcie *ep = &g_pcie[ch_num];
	u32 count = 0, ret = 0, val = 0;
	unsigned long flags;

	spin_lock_irqsave(&ep->pcie_l1_exit_lock, flags);

	if (ep->l1ss_ctrl_id_state == 0) {
		/* Set s/w L1 exit mode */
		exynos_elb_writel(ep, 0x1, PCIE_APP_REQ_EXIT_L1);
		val = exynos_elb_readl(ep, PCIE_APP_REQ_EXIT_L1_MODE);
		val &= ~APP_REQ_EXIT_L1_MODE;
		exynos_elb_writel(ep, val, PCIE_APP_REQ_EXIT_L1_MODE);

		/* Max timeout = 3ms (300 * 10us) */
		while (count < MAX_L1_EXIT_TIMEOUT) {
			val = exynos_elb_readl(ep,
		...
		val |= APP_REQ_EXIT_L1_MODE;
		exynos_elb_writel(ep, val, PCIE_APP_REQ_EXIT_L1_MODE);
		exynos_elb_writel(ep, 0x0, PCIE_APP_REQ_EXIT_L1);
	} else {
		/* remove too much print */
		/* pr_err("%s: skip!!! l1.2 is already diabled(id = 0x%x)\n",
		 * 		   __func__, ep->l1ss_ctrl_id_state); */
	}

	spin_unlock_irqrestore(&ep->pcie_l1_exit_lock, flags);
		...
		unsigned long speed, void *v)
{
	...
	printk("%s - speed : %ld, l1ss_enable = %d\n", __func__, speed, ep->l1ss_enable);
	if (speed > TPUT_THRESHOLD && ep->l1ss_enable == 1) {
		if(exynos_pcie_set_l1ss(0, pp, PCIE_L1SS_CTRL_ARGOS) == 0)
			ep->l1ss_enable = 0;
		else
			ep->l1ss_enable = 1;
	} else if (speed <= TPUT_THRESHOLD && ep->l1ss_enable == 0) {
...
static int chk_pcie_link(struct exynos_pcie *ep)
{
	u32 val;
	int test_result = 0;

	exynos_pcie_poweron(ep->ch_num);
	val = exynos_elb_readl(ep,
					PCIE_ELBI_RDLH_LINKUP) & 0x1f;
...
static int chk_dbi_access(struct exynos_pcie *ep)
{
	u32 val;
	int test_result = 0;
...
static int chk_epconf_access(struct exynos_pcie *ep)
{
	u32 val;
	int test_result = 0;
	struct pcie_port *pp = &ep->pp;
...
static int chk_epmem_access(struct exynos_pcie *ep)
{
	u32 val;
	int test_result = 0;
	struct pcie_port *pp = &ep->pp;
...
static int chk_l12_mode(struct exynos_pcie *ep)
{
	u32 val, counter;
	int test_result = 0;

	pr_info("Wait for flush workqueue...\n");
	flush_delayed_work(&ep->l1ss_boot_delay_work);
	...
	val = readl(ep->phy_base + PHY_PLL_STATE);
...
static int chk_link_recovery(struct exynos_pcie *ep)
{
	u32 val;
	int test_result = 0;
	...
	pr_info("%s: Before set perst, gpio val = %d\n",
		__func__, gpio_get_value(ep->perst_gpio));
	...
	if (val >= 0x0d && val <= 0x14) {
		 ...
		 */
		exynos_pcie_poweroff(ep->ch_num);
		exynos_pcie_poweron(ep->ch_num);
		val = exynos_elb_readl(ep,
				PCIE_ELBI_RDLH_LINKUP) & 0x1f;
		if (val >= 0x0d && val <= 0x14) {
...
static int chk_pcie_dislink(struct exynos_pcie *ep)
{
	u32 val;
	int test_result = 0;
		...
		const char *buf, size_t count)
{
	...
	case 0:
		...
		if (chk_link_recovery(ep)) {
			dev_info(dev, "PCIe UNIT test FAIL[6/7]!!!\n");
			break;
		}

		/* PCIe DisLink Test */
		if (chk_pcie_dislink(ep)) {
	...
	case 9:
		do {
			...
			/* DBI access */
			if (chk_hard_access(ep->rc_dbi_base)) {
							...
							int event)
{
	...
	if (ep->event_reg && ep->event_reg->callback &&
			(ep->event_reg->events & event)) {
		struct exynos_pcie_notify *notify =
			&ep->event_reg->notify;
		notify->event = event;
		notify->user = ep->event_reg->user;
		dev_info(pp->dev, "Callback for the event : %d\n", event);
		ep->event_reg->callback(notify);
	} else {
...
int exynos_pcie_dump_link_down_status(int ch_num)
{
	...
	if (ep->state == STATE_LINK_UP) {
		dev_info(pp->dev, "LTSSM: 0x%08x\n",
			readl(ep->elbi_base + PCIE_ELBI_RDLH_LINKUP));
		dev_info(pp->dev, "LTSSM_H: 0x%08x\n",
			readl(ep->elbi_base + PCIE_CXPL_DEBUG_INFO_H));
		dev_info(pp->dev, "DMA_MONITOR1: 0x%08x\n",
			readl(ep->elbi_base + PCIE_DMA_MONITOR1));
		dev_info(pp->dev, "DMA_MONITOR2: 0x%08x\n",
			readl(ep->elbi_base + PCIE_DMA_MONITOR2));
		dev_info(pp->dev, "DMA_MONITOR3: 0x%08x\n",
			readl(ep->elbi_base + PCIE_DMA_MONITOR3));
	} else
				...
				unsigned long event, void *data)
{
	...
	ret = exynos_pcie_dump_link_down_status(ep->ch_num);
	if (ep->state == STATE_LINK_UP)
		exynos_pcie_register_dump(ep->ch_num);
...
{
	struct exynos_pcie *ep = to_exynos_pcie(pp);
	struct exynos_pcie_clks	*clks = &ep->clks;
	 ...
	 */
	total_clk_num = ep->pcie_clk_num + ep->phy_clk_num;

	for (i = 0; i < total_clk_num; i++) {
		if (i < ep->pcie_clk_num) {
...
void exynos_pcie_print_link_history(struct pcie_port *pp)
{
	...
	for (i = 31; i >= 0; i--)
		history_buffer[i] = exynos_elb_readl(ep,
				PCIE_HISTORY_REG(i));
	for (i = 31; i >= 0; i--)
...
static void exynos_pcie_setup_rc(struct pcie_port *pp)
{
	...
	exynos_pcie_wr_own_conf(pp, PCI_DEVICE_ID, 2,
			    PCI_DEVICE_ID_EXYNOS + ep->ch_num);
	...
	if (ep->ip_ver >= 0x981000 && ep->ch_num == 1)
...
retry:
	...
	if (ep->phy_ops.phy_check_rx_elecidle != NULL)
		ep->phy_ops.phy_check_rx_elecidle(
				ep->phy_pcs_base, IGNORE_ELECIDLE,
				ep->ch_num);

	exynos_elb_writel(ep, 0x0, PCIE_SOFT_CORE_RESET);
	udelay(20);
	exynos_elb_writel(ep, 0x1, PCIE_SOFT_CORE_RESET);
	...
	exynos_elb_writel(ep, val, PCIE_APP_REQ_EXIT_L1_MODE);
	exynos_elb_writel(ep, PCIE_LINKDOWN_RST_MANUAL,
			  PCIE_LINKDOWN_RST_CTRL_SEL);

	/* Q-Channel support and L1 NAK enable when AXI pending */
	val = exynos_elb_readl(ep, PCIE_QCH_SEL);
	if (ep->ip_ver >= 0x889500) {
	...
	exynos_elb_writel(ep, val, PCIE_QCH_SEL);

	exynos_pcie_assert_phy_reset(pp);

	if (ep->ip_ver == 0x889000) {
		exynos_elb_writel(ep, 0x1, PCIE_L1_BUG_FIX_ENABLE);
	}
	...
	if (ep->phy_ops.phy_check_rx_elecidle != NULL)
		ep->phy_ops.phy_check_rx_elecidle(
				ep->phy_pcs_base, ENABLE_ELECIDLE,
				ep->ch_num);

	dev_info(dev, "D state: %x, %x\n",
		 exynos_elb_readl(ep, PCIE_PM_DSTATE) & 0x7,
		 exynos_elb_readl(ep, PCIE_ELBI_RDLH_LINKUP));
	...
	if (count >= MAX_TIMEOUT) {
		...
		if (try_cnt < 10) {
			...
			dev_info(dev, "%s: Set PERST to LOW, gpio val = %d\n",
				__func__,
				gpio_get_value(ep->perst_gpio));
			/* LTSSM disable */
			exynos_elb_writel(ep, PCIE_ELBI_LTSSM_DISABLE,
					  PCIE_APP_LTSSM_ENABLE);
		...
		val = exynos_elb_readl(ep, PCIE_IRQ_EN_PULSE);
...
void exynos_pcie_dislink_work(struct work_struct *work)
{
	...
	if (ep->state == STATE_LINK_DOWN)
		return;

	exynos_pcie_print_link_history(pp);
	exynos_pcie_dump_link_down_status(ep->ch_num);
	exynos_pcie_register_dump(ep->ch_num);
...
void exynos_pcie_work_l1ss(struct work_struct *work)
{
	...
	if (ep->work_l1ss_cnt == 0) {
		dev_info(dev, "[%s]boot_cnt: %d, work_l1ss_cnt: %d\n",
				__func__, ep->boot_cnt,
				ep->work_l1ss_cnt);
...
static void exynos_pcie_assert_phy_reset(struct pcie_port *pp)
{
	...
	ret = exynos_pcie_phy_clock_enable(&ep->pp, PCIE_ENABLE_CLOCK);
	dev_err(pp->dev, "phy clk enable, ret value = %d\n", ret);

	if (ep->phy_ops.phy_config != NULL)
		ep->phy_ops.phy_config(ep->phy_base,
				ep->phy_pcs_base,
				ep->sysreg_base,
				ep->elbi_base, ep->ch_num);

	if (ep->use_ia) {
		void __iomem *ia_base = ep->ia_base;

		dev_info(pp->dev, "Set I/A for CDR reset.");
		/* PCIE_SUB_CON setting */
		writel(0x10, ep->elbi_base + 0x34); /* Enable TOE */
	...
	/* Bus number enable */
	val = exynos_elb_readl(ep, PCIE_SW_WAKE);
	val &= ~(0x1 << 1);
	exynos_elb_writel(ep, val, PCIE_SW_WAKE);
}
...
static void exynos_pcie_enable_interrupts(struct pcie_port *pp)
{
	...
	exynos_elb_writel(ep, 0x0, PCIE_IRQ_TOE_EN_PULSE2);
	/* disable TOE LEVEL interrupt */
	exynos_elb_writel(ep, 0x0, PCIE_IRQ_TOE_EN_LEVEL);

	/* disable LEVEL interrupt */
	exynos_elb_writel(ep, 0x0, PCIE_IRQ_EN_LEVEL);
	/* disable SPECIAL interrupt */
	exynos_elb_writel(ep, 0x0, PCIE_IRQ_EN_SPECIAL);
}
...
static irqreturn_t exynos_pcie_irq_handler(int irq, void *arg)
{
	...
	if (val & IRQ_LINK_DOWN) {
		...
		ep->state = STATE_LINK_DOWN_TRY;
		queue_work(ep->pcie_wq,
				&ep->dislink_work.work);
	}
#ifdef CONFIG_PCI_MSI
	if (val & IRQ_MSI_RISING_ASSERT && ep->use_msi) {
	...
	val = exynos_elb_readl(ep, PCIE_IRQ_LEVEL);
	exynos_elb_writel(ep, val, PCIE_IRQ_LEVEL);

	/* handle SPECIAL interrupt */
	val = exynos_elb_readl(ep, PCIE_IRQ_SPECIAL);
	exynos_elb_writel(ep, val, PCIE_IRQ_SPECIAL);
				...
				u32 *val)
{
	...
	spin_lock_irqsave(&ep->reg_lock, flags);

#ifdef CONFIG_DYNAMIC_PHY_OFF
	ret = regmap_read(ep->pmureg,
			PCIE_PHY_CONTROL + ep->ch_num * 4,
			&reg_val);
	if (reg_val == 0 || ret != 0) {
	...
	if (is_linked == 0) {
		...
		exynos_pcie_phy_clock_enable(&ep->pp,
				PCIE_ENABLE_CLOCK);

#ifdef NCLK_OFF_CONTROL
		if (ep->ip_ver == 0x981000)
			exynos_elb_writel(ep, 0x0, PCIE_L12ERR_CTRL);
#endif
		if (ep->phy_ops.phy_check_rx_elecidle != NULL)
			ep->phy_ops.phy_check_rx_elecidle(
				ep->phy_pcs_base, IGNORE_ELECIDLE,
	...
	ret = dw_pcie_cfg_read(ep->rc_dbi_base + (where), size, val);
...
#ifdef NCLK_OFF_CONTROL
		if (ep->ip_ver == 0x981000)
			exynos_elb_writel(ep, (0x1 << NCLK_OFF_OFFSET),
							PCIE_L12ERR_CTRL);
#endif
		exynos_pcie_phy_clock_enable(pp, PCIE_DISABLE_CLOCK);
	...
	spin_unlock_irqrestore(&ep->reg_lock, flags);
				...
				u32 val)
{
	...
	ret = dw_pcie_cfg_write(ep->rc_dbi_base + (where), size, val);
...
static int exynos_pcie_link_up(struct pcie_port *pp)
{
	...
	val = exynos_elb_readl(ep, PCIE_ELBI_RDLH_LINKUP) & 0x1f;
				...
				struct platform_device *pdev)
{
	...
	exynos_pcie_setup_rc(pp);
	spin_lock_init(&ep->pcie_l1_exit_lock);
	spin_lock_init(&ep->conf_lock);
	spin_lock_init(&ep->reg_lock);	
...
static int exynos_pcie_parse_dt(struct device_node *np, struct pcie_port *pp)
{
	...
	if (of_property_read_u32(np, "ip-ver", &ep->ip_ver)) {
	...
	if (of_property_read_u32(np, "max-link-speed",
		...
		ep->max_link_speed = LINK_SPEED_GEN1;
	}
	...
	if (!of_property_read_string(np, "use-cache-coherency",
		...
		if (!strcmp(use_cache_coherency, "true")) {
			...
			ep->use_cache_coherency = true;
		} else if (!strcmp(use_cache_coherency, "false")) {
			ep->use_cache_coherency = false;
		} else {
	...
	if (ep->int_min_lock)
		pm_qos_add_request(&exynos_pcie_int_qos[ep->ch_num],
	...
	if (IS_ERR(ep->sysreg) && IS_ERR(ep->sysreg_base)) {
...
static int __init exynos_pcie_probe(struct platform_device *pdev)
{
	...
	ep->ch_num = ch_num;
	ep->l1ss_enable = 1;
	ep->state = STATE_LINK_DOWN;

	ep->linkdown_cnt = 0;
	ep->boot_cnt = 0;
	ep->work_l1ss_cnt = 0;
	ep->l1ss_ctrl_id_state = 0;
	ep->atu_ok = 0;
	...
	ep->perst_gpio = of_get_gpio(np, 0);
	if (ep->perst_gpio < 0) {
		dev_err(&pdev->dev, "cannot get perst_gpio\n");
	} else {
		ret = devm_gpio_request_one(pp->dev, ep->perst_gpio,
					    GPIOF_OUT_INIT_LOW,
					    dev_name(pp->dev));
		if (ret)
			goto probe_fail;
	...
	/* Get pin state */
	ep->pcie_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(ep->pcie_pinctrl)) {
		dev_err(&pdev->dev, "Can't get pcie pinctrl!!!\n");
		goto probe_fail;
	}

	ep->pin_state[PCIE_PIN_DEFAULT] =
		pinctrl_lookup_state(ep->pcie_pinctrl, "default");
	if (IS_ERR(ep->pin_state[PCIE_PIN_DEFAULT])) {
		dev_err(&pdev->dev, "Can't set pcie clkerq to output high!\n");
		goto probe_fail;
	}
	ep->pin_state[PCIE_PIN_IDLE] =
		pinctrl_lookup_state(ep->pcie_pinctrl, "idle");
	if (IS_ERR(ep->pin_state[PCIE_PIN_IDLE]))
		dev_err(&pdev->dev, "No idle pin state(but it's OK)!!\n");
	else
		pinctrl_select_state(ep->pcie_pinctrl,
				ep->pin_state[PCIE_PIN_IDLE]);
	...
	ep->elbi_base = devm_ioremap_resource(&pdev->dev, temp_rsc);
	...
	ep->phy_base = devm_ioremap_resource(&pdev->dev, temp_rsc);
	...
	ep->sysreg_base = devm_ioremap_resource(&pdev->dev, temp_rsc);
	if (IS_ERR(ep->sysreg_base)) {
		ret = PTR_ERR(ep->sysreg_base);
		goto probe_fail;
	}

	temp_rsc = platform_get_resource_byname(pdev, IORESOURCE_MEM, "dbi");
	ep->rc_dbi_base = devm_ioremap_resource(&pdev->dev, temp_rsc);
	if (IS_ERR(ep->rc_dbi_base)) {
		ret = PTR_ERR(ep->rc_dbi_base);
		goto probe_fail;
	}

	pp->dbi_base = ep->rc_dbi_base;

	temp_rsc = platform_get_resource_byname(pdev, IORESOURCE_MEM, "pcs");
	ep->phy_pcs_base = devm_ioremap_resource(&pdev->dev, temp_rsc);
	if (IS_ERR(ep->phy_pcs_base)) {
		ret = PTR_ERR(ep->phy_pcs_base);
		goto probe_fail;
	}
	...
	if (ep->idle_ip_index < 0) {
		dev_err(&pdev->dev, "Cant get idle_ip_dex!!!\n");
		ret = -EINVAL;
		goto probe_fail;
	}
	exynos_update_ip_idle_status(ep->idle_ip_index, PCIE_IS_IDLE);
	ep->power_mode_nb.notifier_call = exynos_pci_power_mode_event;
	ep->power_mode_nb.next = NULL;
	ep->power_mode_nb.priority = 0;

	ret = exynos_pm_register_notifier(&ep->power_mode_nb);
	if (ret) {
	...
	ep->ss_dma_mon_nb.notifier_call = exynos_pcie_dma_mon_event;
	ep->ss_dma_mon_nb.next = NULL;
	ep->ss_dma_mon_nb.priority = 0;
	atomic_notifier_chain_register(&panic_notifier_list,
				&ep->ss_dma_mon_nb);
#endif

	ep->pcie_wq = create_freezable_workqueue("pcie_wq");
	if (IS_ERR(ep->pcie_wq)) {
		dev_err(pp->dev, "couldn't create workqueue\n");
		ret = EBUSY;
		goto probe_fail;
	}
	INIT_DELAYED_WORK(&ep->dislink_work,
				exynos_pcie_dislink_work);

	INIT_DELAYED_WORK(&ep->l1ss_boot_delay_work,
...
#ifdef CONFIG_CPU_IDLE
	exynos_pm_unregister_notifier(&ep->power_mode_nb);
#endif
...
static void exynos_pcie_resumed_phydown(struct pcie_port *pp)
{
	...
	/* phy all power down */
	if (ep->phy_ops.phy_all_pwrdn != NULL) {
		ep->phy_ops.phy_all_pwrdn(ep->phy_base,
				ep->phy_pcs_base,
				ep->sysreg_base, ep->ch_num);
	}
...
int exynos_pcie_poweron(int ch_num)
{
	...
	if (ep->state == STATE_LINK_DOWN) {
		...
		regmap_update_bits(ep->pmureg,
			WAKEUP_MASK,
			(0x1 << (WAKEUP_MASK_PCIE_WIFI + ep->ch_num)),
			(0x0 << (WAKEUP_MASK_PCIE_WIFI + ep->ch_num)));
#ifdef CONFIG_CPU_IDLE
		if (ep->use_sicd)
		...
		if (ep->phy_ops.phy_all_pwrdn_clear != NULL) {
			ep->phy_ops.phy_all_pwrdn_clear(
					ep->phy_base,
					ep->phy_pcs_base,
					ep->sysreg_base,
					ep->ch_num);
		...
		ep->state = STATE_LINK_UP_TRY;

		/* Enable history buffer */
		val = exynos_elb_readl(ep, PCIE_STATE_HISTORY_CHECK);
		exynos_elb_writel(ep, 0, PCIE_STATE_HISTORY_CHECK);
		val = HISTORY_BUFFER_ENABLE;
		exynos_elb_writel(ep, val, PCIE_STATE_HISTORY_CHECK);

		exynos_elb_writel(ep, 0x200000, PCIE_STATE_POWER_S);
		exynos_elb_writel(ep, 0xffffffff, PCIE_STATE_POWER_M);
		...
		if (!ep->probe_ok) {
			...
			ep->pci_dev = pci_get_device(vendor_id,
							device_id, NULL);
			if (!ep->pci_dev) {
			...
			pci_rescan_bus(ep->pci_dev->bus);

			/* L1.2 ASPM enable */
			exynos_pcie_config_l1ss(&ep->pp);

#ifdef CONFIG_PCI_MSI
			exynos_pcie_msi_init(pp);
#endif

			if (pci_save_state(ep->pci_dev)) {
				dev_err(pp->dev, "Failed to save pcie state\n");
				goto poweron_fail;
			}
			ep->pci_saved_configs =
				pci_store_saved_state(ep->pci_dev);
			ep->probe_ok = 1;
		} else if (ep->probe_ok) {
			if (ep->boot_cnt == 0) {
				schedule_delayed_work(
					&ep->l1ss_boot_delay_work,
			...
			exynos_pcie_msi_init(pp);
#endif
			if (pci_load_saved_state(ep->pci_dev,
					     ep->pci_saved_configs)) {
				dev_err(pp->dev, "Failed to load pcie state\n");
				goto poweron_fail;
			}
			pci_restore_state(ep->pci_dev);
...
void exynos_pcie_poweroff(int ch_num)
{
	...
	if (ep->state == STATE_LINK_UP ||
		...
		spin_lock_irqsave(&ep->conf_lock, flags);
		exynos_pcie_send_pme_turn_off(ep);
		ep->state = STATE_LINK_DOWN;
		...
		writel(PCIE_ELBI_LTSSM_DISABLE,
				ep->elbi_base + PCIE_APP_LTSSM_ENABLE);

		/* phy all power down */
		if (ep->phy_ops.phy_all_pwrdn != NULL) {
			ep->phy_ops.phy_all_pwrdn(
					ep->phy_base,
					ep->phy_pcs_base,
					ep->sysreg_base,
					ep->ch_num);
...
void exynos_pcie_send_pme_turn_off(struct exynos_pcie *ep)
{
	...
	val = readl(ep->elbi_base + PCIE_ELBI_RDLH_LINKUP) & 0x1f;
...
retry_pme_turnoff:
	...
	exynos_elb_writel(ep, 0x1, XMIT_PME_TURNOFF);

	while (count < MAX_TIMEOUT) {
		if ((exynos_elb_readl(ep, PCIE_IRQ_PULSE)
						& IRQ_RADM_PM_TO_ACK)) {
	...
	exynos_elb_writel(ep, 0x0, XMIT_PME_TURNOFF);
...
#endif
	if (ep->state == STATE_LINK_DOWN) {
		exynos_pcie_resumed_phydown(&ep->pp);
...
{
	int ret = NOTIFY_DONE;
	struct exynos_pcie *ep = container_of(nb,
...
int exynos_pcie_register_event(struct exynos_pcie_register_event *reg)
{
	...
	ep = to_exynos_pcie(pp);
	if (pp) {
		ep->event_reg = reg;
		dev_info(pp->dev,
				"Event 0x%x is registered for RC %d\n",
				reg->events, ep->ch_num);
	} else {
